...
void projectionOCN_AX1E3(Shell_t* moc, List_m* mocAtt, int depart, int arrivee, List_d* nvDepart, int numMotif, Ashape_t* as3d) {
	...
	for (int i = 0; i < 1/*3*/ && positions->premier; i++) // 3 positions les mieux placés (distance min avec arrivée)
	{
...
void genererChemin(Main_t* m, List_m* mocAtt, Shell_t* mocTraite, int depart, int arrivee, int nbMotif3, int nbMotif4, char* InputFile, Ashape_t* as3d){
